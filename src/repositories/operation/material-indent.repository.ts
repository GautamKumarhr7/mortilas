import { eq } from 'drizzle-orm';
import { db } from '../../db/index.js';
import { materialIndents, MaterialIndent, NewMaterialIndent, materialIndentItems, NewMaterialIndentItem } from '../../models/index.js';

export class MaterialIndentRepository {
  async findAll(): Promise<MaterialIndent[]> {
    return await db.select().from(materialIndents);
  }

  async findById(id: number): Promise<MaterialIndent | undefined> {
    const result = await db.select().from(materialIndents).where(eq(materialIndents.id, id));
    return result[0];
  }

  async createWithItems(data: NewMaterialIndent, items: Omit<NewMaterialIndentItem, 'indentId'>[]): Promise<MaterialIndent> {
    return await db.transaction(async (tx) => {
      const indentResult = await tx.insert(materialIndents).values(data).returning();
      const indent = indentResult[0];

      if (items && items.length > 0) {
        const itemsToInsert = items.map(item => ({
          ...item,
          indentId: indent.id,
          workOrderId: indent.workOrderId,
          availableQty: (item as any).availableQty ?? '0',
          shortageQty: (item as any).shortageQty ?? item.requiredQty?.toString() ?? '0',
          requiredQty: item.requiredQty?.toString() ?? '0'
        }));
        await tx.insert(materialIndentItems).values(itemsToInsert as any);
      }
      return indent;
    });
  }
  async updateWithItems(id: number, data: Partial<NewMaterialIndent>, items: Omit<NewMaterialIndentItem, 'indentId'>[]): Promise<MaterialIndent | undefined> {
    return await db.transaction(async (tx) => {
      const indentResult = await tx.update(materialIndents).set(data).where(eq(materialIndents.id, id)).returning();
      const indent = indentResult[0];

      if (!indent) return undefined;

      // Delete old items
      await tx.delete(materialIndentItems).where(eq(materialIndentItems.indentId, id));

      // Insert new items
      if (items && items.length > 0) {
        const itemsToInsert = items.map(item => ({
          ...item,
          indentId: indent.id,
          workOrderId: indent.workOrderId,
          availableQty: (item as any).availableQty ?? '0',
          shortageQty: (item as any).shortageQty ?? item.requiredQty?.toString() ?? '0',
          requiredQty: item.requiredQty?.toString() ?? '0'
        }));
        await tx.insert(materialIndentItems).values(itemsToInsert as any);
      }
      return indent;
    });
  }

  async delete(id: number): Promise<MaterialIndent | undefined> {
    return await db.transaction(async (tx) => {
      await tx.delete(materialIndentItems).where(eq(materialIndentItems.indentId, id));
      const result = await tx.delete(materialIndents).where(eq(materialIndents.id, id)).returning();
      return result[0];
    });
  }
}
