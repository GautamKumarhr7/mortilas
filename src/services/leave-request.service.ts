...
import { LeaveRequest, NewLeaveRequest } from '../models/leave-request.model.js';
...
export class LeaveRequestService {
  ...
  async createLeaveRequest(leaveRequestData: NewLeaveRequest): Promise<LeaveRequest> {
    ...
    switch (type) {
      case 'CL':
        if (leaveBalance.remainCl < requestedDays) throw new Error('Insufficient Casual Leave balance');
        updatedBalance.remainCl = leaveBalance.remainCl - requestedDays;
        break;
      case 'SL':
        if (leaveBalance.remainSl < requestedDays) throw new Error('Insufficient Sick Leave balance');
        updatedBalance.remainSl = leaveBalance.remainSl - requestedDays;
        break;
      case 'EL':
        if (leaveBalance.remainEl < requestedDays) throw new Error('Insufficient Earned Leave balance');
      ...
      case 'LWP':
        ...
        break;
      case 'Maternity':
        if (leaveBalance.remainMaternity < requestedDays) throw new Error('Insufficient Maternity Leave balance');
        updatedBalance.remainMaternity = leaveBalance.remainMaternity - requestedDays;
        break;
      case 'Paternity':
        if (leaveBalance.remainPaternity < requestedDays) throw new Error('Insufficient Paternity Leave balance');
        updatedBalance.remainPaternity = leaveBalance.remainPaternity - requestedDays;
        break;
      case 'CompOff':
        if (leaveBalance.remainCompOff < requestedDays) throw new Error('Insufficient Compensatory Off balance');
  ...
  async updateLeaveRequest(id: number, leaveRequestData: Partial<NewLeaveRequest>): Promise<LeaveRequest | undefined> {
