...
export class ProjectService {
    ...
    const projectCode = projectData.projectCode
      ...
      : generateProjectCode(
          ...
          await this.projectRepository.findProjectCodesByPrefix(
            buildProjectCodePrefix(projectData.category as string, new Date().getFullYear()),
          ),
        );

    const existingProject = await this.projectRepository.findByProjectCode(projectCode);
    if (existingProject) {
  ...
  async updateProject(id: number, projectData: Partial<NewProject>): Promise<Project | undefined> {
    if (projectData.projectCode) {
      const normalizedProjectCode = normalizeProjectCode(projectData.projectCode);
      const existingProject = await this.projectRepository.findByProjectCode(normalizedProjectCode);
