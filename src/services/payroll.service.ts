...
import { Payroll, NewPayroll } from '../models/payroll.model.js';
...
export class PayrollService {
  ...
  async generatePayroll(input: GeneratePayrollInput): Promise<Payroll> {
    ...
    // ── Earnings ──────────────────────────────────────────────────────
    const hra = parseFloat((basic * 0.20).toFixed(2));                    // 20% of basic
    const conveyance = parseFloat((basic * 0.15).toFixed(2));             // 15% of basic
    const specialAllowance = parseFloat((basic * 0.60).toFixed(2));       // 60% of basic
    ...
    const profTax = input.professionalTax !== undefined
      ? input.professionalTax
      : (input.payMonth === 9 ? 200 : 0); // default ₹200 in September; adjust per state
