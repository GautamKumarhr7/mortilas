...
import { Applicant, NewApplicant } from '../models/applicant.model.js';
...
export class ApplicantService {
  ...
  async shortlist(id: number, shortlistedBy: number): Promise<Applicant | undefined> {
    ...
    if (applicant.status !== 'applied') throw new Error('Only applied candidates can be shortlisted');
  ...
  async scheduleInterview(
    ...
    const applicant = await this.repo.findById(id);
    if (!applicant) throw new Error('Applicant not found');
    if (applicant.status !== 'shortlisted') throw new Error('Only shortlisted candidates can be interviewed');
  ...
  async select(id: number, selectedBy: number): Promise<Applicant | undefined> {
    const applicant = await this.repo.findById(id);
    if (!applicant) throw new Error('Applicant not found');
    if (applicant.status !== 'interview_scheduled') throw new Error('Only interviewed candidates can be selected');
  ...
  async onboard(
    ...
    const applicant = await this.repo.findById(id);
    if (!applicant) throw new Error('Applicant not found');
    if (applicant.status !== 'selected') throw new Error('Only selected candidates can be onboarded');
