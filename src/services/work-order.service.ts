...
import { WorkOrder, NewWorkOrder } from "../models/work-order.model.js";
import {
  generateWorkOrderNo,
  normalizeWorkOrderNo,
} from "../helpers/work-order.helper.js";
...
export class WorkOrderService {
  ...
  async createWorkOrder(
    ...
    const workOrderNo = workOrderData.workOrderNo
      ...
      : generateWorkOrderNo(
          await this.workOrderRepository.findWorkOrderNosByPrefix("WO-"),
        );

    const existingWorkOrder =
      await this.workOrderRepository.findByWorkOrderNo(workOrderNo);
  ...
  async updateWorkOrder(
    ...
    if (workOrderData.workOrderNo) {
      const normalizedWorkOrderNo = normalizeWorkOrderNo(
