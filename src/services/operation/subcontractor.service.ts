...
export class SubcontractorService {
  ...
  async createSubcontractor(subcontractorData: Partial<NewSubcontractor>): Promise<Subcontractor> {
    const subcontractorCode = subcontractorData.subcontractorCode
      ...
      : generateSequentialCode(
          'SUB',
          await this.subcontractorRepository.findSubcontractorCodesByPrefix('SUB'),
          4,
        );

    const existingSubcontractor =
      await this.subcontractorRepository.findBySubcontractorCode(subcontractorCode);
