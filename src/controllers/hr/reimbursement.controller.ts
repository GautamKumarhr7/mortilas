...
import { ReimbursementService } from '../../services/hr/reimbursement.service.js';
...
export class ReimbursementController {
  ...
  getByEmployee = async (req: Request, res: Response, next: NextFunction) => {
    try {
      res.json({
        success: true,
        data: await this.service.getByEmployee(parseInt(req.params.employeeId as string, 10)),
      });
    } catch (e) {
      next(e);
    }
  ...
  managerApprove = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const item = await this.service.managerApprove(
  ...
  financeApprove = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const item = await this.service.financeApprove(
        parseInt(req.params.id as string, 10),
        financeId,
        remarks,
      );
