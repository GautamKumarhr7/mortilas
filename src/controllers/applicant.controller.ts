...
export class ApplicantController {
  ...
  constructor() { this.service = new ApplicantService(); }
  ...
  getById = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!item) { res.status(404).json({ success: false, message: 'Applicant not found' }); return; }
  ...
  getByJobPost = async (req: Request, res: Response, next: NextFunction) => {
    try {
      res.json({ success: true, data: await this.service.getByJobPost(parseInt(req.params.jobPostId as string, 10)) });
    } catch (e) { next(e); }
  };
  ...
  shortlist = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const item = await this.service.shortlist(parseInt(req.params.id as string, 10), shortlistedBy);
      res.json({ success: true, data: item });
  ...
  scheduleInterview = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const item = await this.service.scheduleInterview(
        parseInt(req.params.id as string, 10), new Date(interviewDate), interviewedBy, remarks,
      );
      res.json({ success: true, data: item });
    } catch (e: any) { res.status(400).json({ success: false, message: e.message }); }
  };
  ...
  onboard = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.json({ success: true, data: item, message: 'Candidate onboarded as employee successfully' });
    } catch (e: any) { res.status(400).json({ success: false, message: e.message }); }
  };
}
