...
import { RolePermissionService } from '../../services/authority/role_permission.service.js';
...
export class RolePermissionController {
  ...
  createRolePermission = async (req: Request, res: Response, next: NextFunction) => {
    if (Array.isArray(req.body)) {
      const rolePermissions = await this.rolePermissionService.createMultipleRolePermissions(
        req.body,
      );
  ...
  updateRolePermission = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const rolePermissions = await this.rolePermissionService.updateRolePermissionsByRoleId(
      roleId,
      req.body,
    );
    res.status(200).json({ success: true, data: rolePermissions });
