...
import { SubModuleService } from '../../services/authority/submodule.service.js';
...
export class SubModuleController {
  ...
  getAllSubModules = async (req: Request, res: Response, next: NextFunction) => {
    const submodules = await this.submoduleService.getAllSubModules();
    res.status(200).json({ success: true, data: submodules });
  };

  getSubModuleById = async (req: Request, res: Response, next: NextFunction) => {
